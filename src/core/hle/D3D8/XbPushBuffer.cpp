...
xbox::dword_xt * HLE_get_NV2A_vertex_program_slot_ptr(unsigned slot_address)
{
	NV2AState* dev = g_NV2A->GetDeviceState();
	PGRAPHState* pg = &(dev->pgraph);

	// See CASE_32(NV097_SET_TRANSFORM_PROGRAM, 4) in LLE pgraph_handle_method()
	assert(slot_address <= NV2A_MAX_TRANSFORM_PROGRAM_LENGTH);
	return (xbox::dword_xt*)&(pg->vsh_program_slots[slot_address][0]);
}
