...
sampler2D _sampler2D_0;
sampler2D _sampler2D_1;
sampler2D _sampler2D_2;
sampler2D _sampler2D_3;

sampler3D _sampler3D_0;
sampler3D _sampler3D_1;
sampler3D _sampler3D_2;
sampler3D _sampler3D_3;

samplerCUBE _sampler6F_0;
samplerCUBE _sampler6F_1;
samplerCUBE _sampler6F_2;
samplerCUBE _sampler6F_3;
...
#define Sample2D(st)   tex2D(_sampler2D_ ## st, s.xy)  // Ignores s.z (and whatever it's set to, will be optimized away by the compiler, see [1] below)
#define Sample3D(st)   tex3D(_sampler3D_ ## st, s.xyz)
#define Sample6F(st) texCUBE(_sampler6F_ ## st, s.xyz)
...
/*0123 tex          */ #define PS_TEXTUREMODES_PROJECT2D(st)                                          s = iT[st].xyz;      v = Sample2D(st); t[st] = v // Seems to work (are x/w and y/w implicit?) [1]
/*0123 tex          */ #define PS_TEXTUREMODES_PROJECT3D(st)                                          s = iT[st].xyz;      v = Sample3D(st); t[st] = v // Seems to work (is z/w implicit?)
/*0123 tex          */ #define PS_TEXTUREMODES_CUBEMAP(st)                                            s = iT[st].xyz;      v = Sample6F(st); t[st] = v // TODO : Test
...
/*-123 texbem       */ #define PS_TEXTUREMODES_BUMPENVMAP(st)                                         s = BumpEnv(st);     v = Sample2D(st); t[st] = v // Seems to work
/*-123 texbeml      */ #define PS_TEXTUREMODES_BUMPENVMAP_LUM(st)       PS_TEXTUREMODES_BUMPENVMAP(st);                    v.rgb *= LSO(st); t[st] = v // TODO : Test
/*--23 texbrdf      */ #define PS_TEXTUREMODES_BRDF(st)                                               s = Brdf(st);        v = Sample3D(st); t[st] = v // TODO : Test (t[st-2] is 16 bit eyePhi,eyeSigma; t[st-1] is lightPhi,lightSigma)
/*--23 texm3x2tex   */ #define PS_TEXTUREMODES_DOT_ST(st)               CalcDot(st); n = Normal2(st); s = n;               v = Sample2D(st); t[st] = v // TODO : Test
...
/*--2- texm3x3diff  */ #define PS_TEXTUREMODES_DOT_RFLCT_DIFF(st)       CalcDot(st); n = Normal3(st); s = n;               v = Sample6F(st); t[st] = v // TODO : Test
/*---3 texm3x3vspec */ #define PS_TEXTUREMODES_DOT_RFLCT_SPEC(st)       CalcDot(st); n = Normal3(st); s = Reflect(n, Eye); v = Sample6F(st); t[st] = v // TODO : Test
/*---3 texm3x3tex   */ #define PS_TEXTUREMODES_DOT_STR_3D(st)           CalcDot(st); n = Normal3(st); s = n;               v = Sample3D(st); t[st] = v // TODO : Test
/*---3 texm3x3tex   */ #define PS_TEXTUREMODES_DOT_STR_CUBE(st)         CalcDot(st); n = Normal3(st); s = n;               v = Sample6F(st); t[st] = v // TODO : Test
/*-123 texreg2ar    */ #define PS_TEXTUREMODES_DPNDNT_AR(st)                                          s = src(st).arg;     v = Sample2D(st); t[st] = v // TODO : Test [1]
/*-123 texreg2bg    */ #define PS_TEXTUREMODES_DPNDNT_GB(st)                                          s = src(st).gba;     v = Sample2D(st); t[st] = v // TODO : Test [1]
...
/*---3 texm3x3spec  */ #define PS_TEXTUREMODES_DOT_RFLCT_SPEC_CONST(st) CalcDot(st); n = Normal3(st); s = Reflect(n, c0);  v = Sample6F(st); t[st] = v // TODO : Test
