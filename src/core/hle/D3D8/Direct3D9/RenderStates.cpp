...
void XboxRenderStateConverter::BuildRenderStateMappingTable()
{
    ...
    for (unsigned int RenderState = XTL::X_D3DRS_PSALPHAINPUTS0; RenderState <= XTL::X_D3DRS_LAST; RenderState++) {
        if (XTL::DxbxRenderStateInfo[RenderState].V <= g_LibVersion_D3D8) {
            ...
            EmuLog(LOG_LEVEL::INFO, "%s = %d", XTL::DxbxRenderStateInfo[RenderState].S, XboxIndex);
            XboxIndex++;
            continue;
        }

        EmuLog(LOG_LEVEL::INFO, "%s Not Present", XTL::DxbxRenderStateInfo[RenderState].S);
    }
}
...
void XboxRenderStateConverter::SetXboxRenderState(uint32_t State, uint32_t Value)
{
    if (!XboxRenderStateExists(State)) {
        EmuLog(LOG_LEVEL::WARNING, "Attempt to write a Renderstate (%s) that does not exist in the current D3D8 XDK Version (%d)", XTL::DxbxRenderStateInfo[State].S, g_LibVersion_D3D8);
        return;
    }
...
{
    if (!XboxRenderStateExists(State)) {
        EmuLog(LOG_LEVEL::WARNING, "Attempt to read a Renderstate (%s) that does not exist in the current D3D8 XDK Version (%d)", XTL::DxbxRenderStateInfo[State].S, g_LibVersion_D3D8);
        return 0;
    }
...
void XboxRenderStateConverter::ApplySimpleRenderState(uint32_t State, uint32_t Value)
{
    switch (State) {
        ...
        default:
            ...
            if (XTL::DxbxRenderStateInfo[State].PC != 0) {
                EmuLog(LOG_LEVEL::WARNING, "ApplySimpleRenderState(%s, 0x%.08X) is unimplemented!", XTL::DxbxRenderStateInfo[State].S, Value);
            }
            return;
    }
    ...
    g_pD3DDevice->SetRenderState((XTL::D3DRENDERSTATETYPE)(XTL::DxbxRenderStateInfo[State].PC), Value);
}
...
void XboxRenderStateConverter::ApplyDeferredRenderState(uint32_t State, uint32_t Value)
{
    ...
    switch (State) {
        ...
        default:
            // Only log missing state if it has a PC counterpart
            if (XTL::DxbxRenderStateInfo[State].PC != 0) {
                EmuLog(LOG_LEVEL::WARNING, "ApplyDeferredRenderState(%s, 0x%.08X) is unimplemented!", XTL::DxbxRenderStateInfo[State].S, Value);
            }
            return;
    }
    ...
    g_pD3DDevice->SetRenderState(XTL::DxbxRenderStateInfo[State].PC, Value);
}
...
void XboxRenderStateConverter::ApplyComplexRenderState(uint32_t State, uint32_t Value)
{
    switch (State) {
        ...
        case XTL::X_D3DRS_FILLMODE:
            Value = XTL::EmuXB2PC_D3DFILLMODE(Value);

            if (WireFrameMode > 0) {
                if (WireFrameMode == 1) {
                    Value = XTL::D3DFILL_WIREFRAME;
                } else {
                    Value = XTL::D3DFILL_POINT;
                }
            }
            break;
        case XTL::X_D3DRS_CULLMODE:
            switch (Value) {
                case XTL::X_D3DCULL_NONE: Value = XTL::D3DCULL_NONE; break;
                case XTL::X_D3DCULL_CW: Value = XTL::D3DCULL_CW; break;
                case XTL::X_D3DCULL_CCW: Value = XTL::D3DCULL_CCW;break;
        ...
        default:
            // Only log missing state if it has a PC counterpart
            if (XTL::DxbxRenderStateInfo[State].PC != 0) {
                EmuLog(LOG_LEVEL::WARNING, "ApplyComplexRenderState(%s, 0x%.08X) is unimplemented!", XTL::DxbxRenderStateInfo[State].S, Value);
            }
            return;
    }
