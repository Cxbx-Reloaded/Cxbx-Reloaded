...
void HLE_PushInit()
{
	SwitchToThread(); //with current partial push buffer implimentation, this is need to help sync things up and stop a consistent crash
	// by giving a pause for this thread to allow the others to do what they neeed.
	// Once syncing issues are fixed, via full reading/setting d3d commands from pushbuffer or other method, this can then be removed.
