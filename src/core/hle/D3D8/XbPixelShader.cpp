...
void DxbxUpdateActivePixelShader() // NOPATCH
{
  ...
  if(pgraph_is_NV2A_bumpenv())
	pPSDef = g_pXbox_PixelShader != nullptr ? (xbox::X_D3DPIXELSHADERDEF*)(NV2ARenderStates.GetPixelShaderRenderStatePointer()) : nullptr;
  else
  ...
  if(pgraph_is_NV2A_bumpenv()){
	  ...
	  fColor[PSH_XBOX_CONSTANT_FOG] = NV2ARenderStates.GetXboxRenderState(xbox::X_D3DRS_FOGCOLOR);
	  ...
	  for (int stage_nr = 1; stage_nr < xbox::X_D3DTS_STAGECOUNT; stage_nr++, xbox_bump_stage++) {
		  ...
		  fColor[PSH_XBOX_CONSTANT_BEM + xbox_bump_stage].b = pg->KelvinPrimitive.SetTexture[stage_nr].SetBumpEnvMat11; // Maps to BEM[stage].z
		  fColor[PSH_XBOX_CONSTANT_BEM + xbox_bump_stage].a = pg->KelvinPrimitive.SetTexture[stage_nr].SetBumpEnvMat10; // Maps to BEM[stage].w
		  // Bump map Luminance registers
